...
function evaluatePacket(packetRoot) {
    if (packetRoot.type === 4) {
        return packetRoot.value;

    } else if (packetRoot.type === 0) {
        let total = 0;
        for (let child of packetRoot.children) {
            total += evaluatePacket(child);
        }
        return total;

    } else if (packetRoot.type === 1) {
        let total = 1;
        for (let child of packetRoot.children) {
            total *= evaluatePacket(child);
        }
    ...
    } else if (packetRoot.type === 2) {
        let total = Infinity;
        for (let child of packetRoot.children) {
            total = Math.min(total, evaluatePacket(child));
        }
        return total;

    } else if (packetRoot.type === 3) {
        let total = -Infinity;
        for (let child of packetRoot.children) {
            total = Math.max(total, evaluatePacket(child));
        }
        return total;

    } else if (packetRoot.type === 5) {
        let a = evaluatePacket(packetRoot.children[0]);
        let b = evaluatePacket(packetRoot.children[1]);
        return (a > b) ? 1 : 0;

    } else if (packetRoot.type === 6) {
        let a = evaluatePacket(packetRoot.children[0]);
        let b = evaluatePacket(packetRoot.children[1]);
        return (a < b) ? 1 : 0;

    } else if (packetRoot.type === 7) {
        let a = evaluatePacket(packetRoot.children[0]);
        let b = evaluatePacket(packetRoot.children[1]);
        return (a === b) ? 1 : 0;
